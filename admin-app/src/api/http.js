...
export async function request(path, options = {}) {
  ...
  if (!response.ok) {
    const validationErrors = Array.isArray(payload.errors) && payload.errors.length > 0
      ? `: ${payload.errors.map((issue) => `${issue.path || 'field'} - ${issue.message}`).join(', ')}`
      : '';

    throw new Error(`${payload.message || 'Request failed'}${validationErrors}`);
  }
...
export async function requestJson(path, method, data, headers = {}) {
  return request(path, {
    method,
    headers,
